...
const (
	...
	AwardTypeRanked                       = "ranked"
	AwardTypeSponsor                      = "sponsor"
...
func updateAwardPostgres(ctx *config.AppContext, awardID string, in AwardInput) error {
	...
	if in.AwardType != AwardTypeSponsor {
		if _, err := tx.Exec(dbctx, `
...
func listAwardsForCompetitionByArchiveStatePostgres(ctx *config.AppContext, competitionID string, archived bool) ([]*types.Award, error) {
	...
	orderBy := "CASE WHEN award_type = 'ranked' AND award_rank IS NOT NULL THEN 0 WHEN award_type = 'ranked' THEN 1 ELSE 2 END, award_rank NULLS LAST, title, id"
	if archived {
...
func assignProjectAwardPostgres(ctx *config.AppContext, awardID, projectID string) error {
	...
	if err := tx.QueryRow(dbctx, `
		SELECT competition_id::text, max_awardees, finalists_only
		FROM awards
		WHERE id::text = $1
			AND archived_at IS NULL
		FOR UPDATE
	`, awardID).Scan(&awardCompetitionID, &maxAwardees, &finalistsOnly); err != nil {
...
func removeProjectAwardPostgres(ctx *config.AppContext, awardID, projectID string) error {
	...
	if err := tx.QueryRow(dbctx, `
		...
		WHERE id::text = $1
			AND archived_at IS NULL
		FOR UPDATE
	`, awardID).Scan(&lockedAwardID, &awardCompetitionID); err != nil {
...
func addAwardJudgePostgres(ctx *config.AppContext, awardID, personID string) error {
	...
	`, awardID, personID, AwardTypeSponsor)
	if err != nil {
...
func upsertAwardVotePostgres(ctx *config.AppContext, in AwardVoteInput) error {
	...
	commandTag, err := ctx.DB.Exec(context.Background(), `
		...
		DO UPDATE SET
			...
			submitted_at = EXCLUDED.submitted_at,
			updated_at = now()
	`, in.AwardID, in.JudgePersonID, in.ProjectID, in.Notes, AwardTypeSponsor)
	if err != nil {
...
func normalizeAwardInput(in AwardInput) AwardInput {
	...
	if in.AwardType != AwardTypeRanked {
		in.AwardRank = nil
	}
...
func normalizeAwardType(value string) string {
	switch strings.ToLower(strings.TrimSpace(value)) {
	case AwardTypeSponsor:
		return AwardTypeSponsor
	default:
		return AwardTypeRanked
	}
}
