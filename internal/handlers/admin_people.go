...
func AdminPersonMerge(w http.ResponseWriter, r *http.Request, ctx *config.AppContext) {
	...
	preview, err := getters.PreviewPersonMerge(ctx, r.URL.Query().Get("canonical"), r.URL.Query().Get("source"))
	if err != nil {
		http.Redirect(w, r, "/admin/people?error="+url.QueryEscape(err.Error()), http.StatusSeeOther)
		return
	}
	renderAdminPersonMerge(w, ctx, adminPersonMergePage(preview, ""))
}
...
func AdminPersonMergeSave(w http.ResponseWriter, r *http.Request, ctx *config.AppContext) {
	...
	if err != nil {
		renderAdminPersonMerge(w, ctx, adminPersonMergePage(preview, err.Error()))
		return
	}
	if len(preview.Conflicts) > 0 {
		renderAdminPersonMerge(w, ctx, adminPersonMergePage(preview, "Resolve every relationship conflict before merging."))
		return
	}
